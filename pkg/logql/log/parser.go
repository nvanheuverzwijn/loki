...
func (l *SyslogParser) Process(line []byte, lbs *LabelsBuilder) ([]byte, bool) {
	...
	if b == '<' {
		nontransparent.NewParser(syslog.WithListener(l.handleMessage), syslog.WithMaxMessageLength(l.maxMessageLength), syslog.WithBestEffort()).Parse(r)
	} else if b >= '0' && b <= '9' {
		octetcounting.NewParser(syslog.WithListener(l.handleMessage), syslog.WithMaxMessageLength(l.maxMessageLength), syslog.WithBestEffort()).Parse(r)
	} else {
		return line, false
	}
	return line, true
...
func (l *SyslogParser) handleMessage(msg *syslog.Result) {
	if msg.Error != nil {
		if msg.Message == nil {
			l.lbs.SetErr(errSyslog) // msg.Error.Error()
			return
		}
